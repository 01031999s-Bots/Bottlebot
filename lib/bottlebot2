...
=end
	...
	self.send('MSG',message['channel'],msg)
		...
		msg = "\nActive Commands:\n"
		msg += " roulette, draw 1-3, & deal"
		msg += "\nBeta commands:\n"
		msg += " slap, adminme(only works for those on the current admin list, listed below. ((if you wish to be added, message Slave Sex Sure and she'll test your worth!))) , ownerme(again admin list)"
	self.send('MSG',message['channel'],msg) 
=begin if message['gender'] =~ /^male/
	elsif message['message'] =~ /^fuckme/
		player = message['character']
		msg = "/me saunders over to " + message['character'] + " allowing him to plunge into my juicy depths!"
	self.send('MSG',message['channel'],msg)
elsif message['gender'] =~ /^female/
	if message['message'] =~ /^fuckme/
		msg = "/me saunders over to " + message['character'] + " a dick sprouting forth which i use to plunge into her juicy depths!"
	self.send('MSG',message['channel'],msg)
end
end
=end
